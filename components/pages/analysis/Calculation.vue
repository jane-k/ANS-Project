...
export default {
  ...
  data() {
    return {
      // ANSDataTemplate: this.ANSDataTemplate,
    };
  },
  computed: {
    ...
    calculateANS(Cetha = 1) {
      ...
      var a = Math.log(
        this.ANSDatabase.FTRgoal_Start.value /
          (0.999 * this.ANSDatabase.FTRgoal_Fin.value)
      ...
      for (let t = 0; t < YEAR; t++) {
        ...
        this.ANSDataTemplate.Flt_Sav.value[t] =
          (this.ANSDatabase.FTRgoal_Fin.value * 0.999) /
      ...
      for (let l = 0; l < N_AC; l++) {
        ...
        for (let k = 0; k < YEAR; k++) {
          ...
          Temp_CA =
            Temp_CA +
            this.ANSDatabase.FCE_flight.value[l][k] *
              this.ANSDatabase.FU_flight.value[l][k];
          Temp_CB =
            Temp_CB +
            this.ANSDatabase.ACE_TO_flight.value[l][k] *
              this.ANSDatabase.AU_flight.value[l][k];
          Temp_CC =
            Temp_CC +
            this.ANSDatabase.ACE_LD_flight.value[l][k] *
              this.ANSDatabase.AU_flight.value[l][k];
        }
      ...
      for (let l = 0; l < MAX; l++) {
        ...
        for (let k = 0; k < N_AC; k++) {
          Temp_CA =
            Temp_CA +
            this.ANSDatabase.FFE_flight.value[l][k] *
              this.ANSDatabase.FU_flight.value[l][k];
          Temp_CB =
            Temp_CB +
            this.ANSDatabase.AFE_TO_flight.value[l][k] *
              this.ANSDatabase.AU_flight.value[l][k];
          Temp_CC =
            Temp_CC +
            this.ANSDatabase.AFE_LD_flight.value[l][k] *
              this.ANSDatabase.AU_flight.value[l][k];
        }
      ...
      for (let l = 0; l < MAX; l++) {
        for (let t = 0; t < YEAR; t++) {
          ...
          FTR_DDgoal[l][t] =
            ...
            this.ANSDataTemplate.Flt_Sav.value[t]; // 국내선 이륙 시간 절감
          FTR_ADgoal[l][t] =
            this.ANSDatabase.Time_DLD.value[l] *
            this.ANSDataTemplate.Flt_Sav.value[t]; // 국내선 착륙 시간 절감
          FTR_DRgoal[l][t] =
            this.ANSDatabase.Time_DRoute.value[l] *
            this.ANSDataTemplate.Flt_Sav.value[t]; // 국내선 항로 시간 절감 FTR_RDgoal 추가 정의

          FTR_DIgoal[l][t] =
            this.ANSDatabase.Time_ITO.value[l] *
            this.ANSDataTemplate.Flt_Sav.value[t]; // 국제선 이륙시간 절감
          FTR_DIRgoal[l][t] =
            this.ANSDatabase.Time_DIRoute.value[l] *
            this.ANSDataTemplate.Flt_Sav.value[t]; // 국제선 이륙항로 시간 절감, FTR_DRIgoal, Time_DIRoute 추가정의
          FTR_AIgoal[l][t] =
            this.ANSDatabase.Time_ILD.value[l] *
            this.ANSDataTemplate.Flt_Sav.value[t]; // 국제선 착륙시간 절감
          FTR_AIRgoal[l][t] =
            this.ANSDatabase.Time_AIRoute.value[l] *
            this.ANSDataTemplate.Flt_Sav.value[t]; // 국제선 착륙항로 시간 절감, FTR_ARIgoal, Time_AIRoute 추가정의

          // 운항편수를 고려한 총운항시간 절감규모 산출하기 : 국내선, 국제선 출발, 국제선 도착으로 구분
          this.ANSDataTemplate.FTR_DDtotal.value[l][t] =
            this.ANSDatabase.N_DD_Flght.value[l][t] * FTR_DDgoal[l][t]; // 국내선 이륙과정 총절감시간 (출발)
          this.ANSDataTemplate.FTR_ADtotal.value[l][t] =
            this.ANSDatabase.N_AD_Flght.value[l][t] * FTR_ADgoal[l][t]; // 국내선 착과정 총절감시간 (도착)
          this.ANSDataTemplate.FTR_DRtotal.value[l][t] =
            ((this.ANSDatabase.N_DD_Flght.value[l][t] +
              this.ANSDatabase.N_AD_Flght.value[l][t]) /
              2) *
            FTR_DDgoal[l][t]; // 국내선 항로과정 총절감시간 FTR_DRtotal 추가정의
          this.ANSDataTemplate.FTR_DItotal.value[l][t] =
            this.ANSDatabase.N_DI_Flght.value[l][t] * FTR_DIgoal[l][t]; // 국제선 이륙과정 총절감시간
          this.ANSDataTemplate.FTR_DIRtotal.value[l][t] =
            this.ANSDatabase.N_DI_Flght.value[l][t] * FTR_DIRgoal[l][t]; // 국제선 이륙-항로과정 총절감시간  FTR_DIRtotal 추가정의
          this.ANSDataTemplate.FTR_AItotal.value[l][t] =
            this.ANSDatabase.N_AI_Flght.value[l][t] * FTR_AIgoal[l][t]; // 국제선 착률과정 총절감시간
          this.ANSDataTemplate.FTR_AIRtotal.value[l][t] =
            this.ANSDatabase.N_DI_Flght.value[l][t] * FTR_AIRgoal[l][t]; // 국제선 착륙-항로과정 총절감시간  FTR_AIRtotal 추가정의
        }
      }
      ...
      for (let l = 0; l < MAX; l++) {
        for (let t = 0; t < YEAR; t++) {
          this.ANSDataTemplate.CER_DDamount.value[l][t] =
            ACE_TO_hour[l] * this.ANSDataTemplate.FTR_DDtotal.value[l][t]; // 국내선 출발 총탄소배출량
          this.ANSDataTemplate.CER_ADamount.value[l][t] =
            ACE_LD_hour[l] * this.ANSDataTemplate.FTR_ADtotal.value[l][t]; // 국내선 착륙 총탄소배출량
          this.ANSDataTemplate.CER_DRamount.value[l][t] =
            FCE_hour[l] * this.ANSDataTemplate.FTR_DRtotal.value[l][t]; // 국내선 항로 총탄소배출량  CER_DRamount 추가정의

          this.ANSDataTemplate.CER_DIamount.value[l][t] =
            ACE_TO_hour[l] * this.ANSDataTemplate.FTR_DItotal.value[l][t]; // 국제선 출발 총탄소배출량
          this.ANSDataTemplate.CER_DIRamount.value[l][t] =
            FCE_hour[l] * this.ANSDataTemplate.FTR_DIRtotal.value[l][t]; // 국제선 출발-항로 총탄소배출량 CER_DIRamount 추가정의
          this.ANSDataTemplate.CER_AIamount.value[l][t] =
            ACE_LD_hour[l] * this.ANSDataTemplate.FTR_AItotal.value[l][t]; // 국제선 도착 총탄소배출량
          this.ANSDataTemplate.CER_AIRamount.value[l][t] =
            FCE_hour[l] * this.ANSDataTemplate.FTR_AIRtotal.value[l][t]; // 국제선 도착-항로 총탄소배출량  CER_AIRamount 추가정의
        }
      }
      ...
      for (let l = 0; l < MAX; l++) {
        for (let t = 0; t < YEAR; t++) {
          this.ANSDataTemplate.CER_DDcost.value[l][t] =
            this.ANSDatabase.carbonCredit.value[t] *
            this.ANSDataTemplate.CER_DDamount.value[l][t]; // 국내선 출발 총탄소배출 절감비용
          this.ANSDataTemplate.CER_ADcost.value[l][t] =
            this.ANSDatabase.carbonCredit.value[t] *
            this.ANSDataTemplate.CER_ADamount.value[l][t]; // 국내선 착륙 총탄소배출 절감비용
          this.ANSDataTemplate.CER_DRcost.value[l][t] =
            this.ANSDatabase.carbonCredit.value[t] *
            this.ANSDataTemplate.CER_DRamount.value[l][t]; // 국내선 항로 총탄소배출 절감비용  CER_DRcost 추가정의

          this.ANSDataTemplate.CER_DIcost.value[l][t] =
            this.ANSDatabase.carbonCredit.value[t] *
            this.ANSDataTemplate.CER_DIamount.value[l][t]; // 국제선 출발 총탄소배출 절감비용
          this.ANSDataTemplate.CER_DIRcost.value[l][t] =
            this.ANSDatabase.carbonCredit.value[t] *
            this.ANSDataTemplate.CER_DIRamount.value[l][t]; // 국제선 출발-항로 총탄소배출 절감비용  CER_DIRcost[l][t] 추가정의
          this.ANSDataTemplate.CER_AIcost.value[l][t] =
            this.ANSDatabase.carbonCredit.value[t] *
            this.ANSDataTemplate.CER_AIamount.value[l][t]; // 국제선 도착 총탄소배출 절감비용
          this.ANSDataTemplate.CER_AIRcost.value[l][t] =
            this.ANSDatabase.carbonCredit.value[t] *
            this.ANSDataTemplate.CER_AIRamount.value[l][t]; // 국제선 도착-항로 총탄소배출 절감비용   CER_AIRcost  추가정의
        }
      }
      ...
      for (let l = 0; l < MAX; l++) {
        for (let t = 0; t < YEAR; t++) {
          this.ANSDataTemplate.FR_DDamount.value[l][t] =
            AFE_TO_hour[l] * this.ANSDataTemplate.FTR_DDtotal.value[l][t]; // 국내선 출발 총연료절감량
          this.ANSDataTemplate.FR_ADamount.value[l][t] =
            AFE_LD_hour[l] * this.ANSDataTemplate.FTR_ADtotal.value[l][t]; // 국내선 착륙 총연료절감량
          this.ANSDataTemplate.FR_DRamount.value[l][t] =
            FFE_hour[l] * this.ANSDataTemplate.FTR_DRtotal.value[l][t]; // 국내선 항로 총연료절감량  FR_DRamount 추가정의

          this.ANSDataTemplate.FR_DIamount.value[l][t] =
            AFE_TO_hour[l] * this.ANSDataTemplate.FTR_DItotal.value[l][t]; // 국제선 출발 총연료절감량
          this.ANSDataTemplate.FR_DIRamount.value[l][t] =
            FFE_hour[l] * this.ANSDataTemplate.FTR_DIRtotal.value[l][t]; // 국제선 출발-항로 총연료절감량 FR_DIRamount 추가정의
          this.ANSDataTemplate.FR_AIamount.value[l][t] =
            AFE_LD_hour[l] * this.ANSDataTemplate.FTR_AItotal.value[l][t]; // 국제선 도착 총연료절감량
          this.ANSDataTemplate.FR_AIRamount.value[l][t] =
            FFE_hour[l] * this.ANSDataTemplate.FTR_AIRtotal.value[l][t]; // 국제선 도착-항로 총연료절감량  FR_AIRamount 추가정의
        }
      }
      ...
      for (let l = 0; l < MAX; l++) {
        for (let t = 0; t < YEAR; t++) {
          this.ANSDataTemplate.FR_DDcost.value[l][t] =
            this.ANSDatabase.fuelCredit.value[t] *
            this.ANSDataTemplate.CER_DDamount.value[l][t]; // 국내선 출발 총연료비절감
          this.ANSDataTemplate.FR_ADcost.value[l][t] =
            this.ANSDatabase.fuelCredit.value[t] *
            this.ANSDataTemplate.CER_ADamount.value[l][t]; // 국내선 착륙 총연료비절감
          this.ANSDataTemplate.FR_DRcost.value[l][t] =
            this.ANSDatabase.fuelCredit.value[t] *
            this.ANSDataTemplate.CER_DRamount.value[l][t]; // 국내선 항로 총연료비절감  FR_DRcost 추가정의

          this.ANSDataTemplate.FR_DIcost.value[l][t] =
            this.ANSDatabase.fuelCredit.value[t] *
            this.ANSDataTemplate.CER_DIamount.value[l][t]; // 국제선 출발 총연료비절감
          this.ANSDataTemplate.FR_DIRcost.value[l][t] =
            this.ANSDatabase.fuelCredit.value[t] *
            this.ANSDataTemplate.CER_DIRamount.value[l][t]; // 국제선 출발-항로 총연료비절감  FR_DIRcost[l][t] 추가정의
          this.ANSDataTemplate.FR_AIcost.value[l][t] =
            this.ANSDatabase.fuelCredit.value[t] *
            this.ANSDataTemplate.CER_AIamount.value[l][t]; // 국제선 도착 총연료비절감
          this.ANSDataTemplate.FR_AIRcost.value[l][t] =
            this.ANSDatabase.fuelCredit.value[t] *
            this.ANSDataTemplate.CER_AIRamount.value[l][t]; // 국제선 도착-항로 총연료비절감   FR_AIRcost  추가정의
        }
      }
        ...
        for (let t = 0; t < YEAR; t++) {
          this.ANSDataTemplate.OPR_DDcost.value[l][t] =
            (this.ANSDataTemplate.FTR_DDtotal.value[l][t] +
              this.ANSDataTemplate.FTR_ADtotal.value[l][t] +
              this.ANSDataTemplate.FTR_DRtotal.value[l][t]) *
            this.ANSDatabase.OP_DDcost.value[t]; // 국내선 운항비용 절감
          this.ANSDataTemplate.OPR_DIcost.value[l][t] =
            (this.ANSDataTemplate.FTR_DItotal.value[l][t] +
              this.ANSDataTemplate.FTR_DIRtotal.value[l][t]) *
            this.ANSDatabase.OP_DIcost.value[t]; // 국제선 출발편 운항비용 절감
          this.ANSDataTemplate.OPR_ADcost.value[l][t] =
            (this.ANSDataTemplate.FTR_ADtotal.value[l][t] +
              this.ANSDataTemplate.FTR_AIRtotal.value[l][t]) *
            this.ANSDatabase.OP_AIcost.value[t]; // 국제선 도착편 운항비용 절감
        }
      }
      ...
      var a = Math.log(
        this.ANSDatabase.Dlygoal_Start.value /
          (0.999 * this.ANSDatabase.Dlygoal_Fin.value)
      ...
      for (let t = 0; t < YEAR; t++) {
        ...
        Dly_Sav[t] =
          (this.ANSDatabase.Dlygoal_Fin.value * 0.999) /
          (Cetha + Math.exp(a + b * t)) /
          100;
      }
      ...
      for (let l = 0; l < MAX; l++) {
        for (let t = 0; t < YEAR; t++) {
          ...
          Avg_DD_Dly[l][t] =
            this.ANSDatabase.Time_DD_Dly.value[l] *
            Dly_Sav[t] *
            this.ANSDatabase.DLY_DD_ANSshare.value[l]; // 국내선 이륙(공항) 지연시간 절감
          Avg_DI_Dly[l][t] =
            this.ANSDatabase.Time_DI_Dly.value[l] *
            Dly_Sav[t] *
            this.ANSDatabase.DLY_DI_ANSshare.value[l]; // 국제선 이륙(공항) 지연시간 절감

          // 총출발지연 감소 시간을 국내선, 국제선으로 구분하여 산출
          this.ANSDataTemplate.TL_DD_Dly.value[l][t] =
            this.ANSDatabase.N_DD_Flght.value[l][t] *
            this.ANSDatabase.N_DD_Flght.value[l][t]; // 국내선 총 출발지연시간 절감
          this.ANSDataTemplate.TL_DI_Dly.value[l][t] =
            this.ANSDatabase.N_DI_Flght.value[l][t] * Avg_DI_Dly[l][t]; // 국제선 총 공항 출발지연시간 절감
        }
      }
      ...
      for (let l = 0; l < MAX; l++) {
        for (let t = 0; t < YEAR; t++) {
          this.ANSDataTemplate.CER_DDamount_byADLY.value[l][t] =
            ACE_TO_hour[l] * this.ANSDataTemplate.TL_DD_Dly.value[l][t]; // 국내선 총 공항 출발 탄소배출 절감량
          this.ANSDataTemplate.CER_DIamount_byADLY.value[l][t] =
            ACE_TO_hour[l] * this.ANSDataTemplate.TL_DI_Dly.value[l][t]; // 국제선 총 공항 출발 탄소배출 절감량
          this.ANSDataTemplate.FR_DDamount_byADLY.value[l][t] =
            AFE_TO_hour[l] * this.ANSDataTemplate.TL_DD_Dly.value[l][t]; // 국내선 총 공항 출발 연료 절감량
          this.ANSDataTemplate.FR_DIamount_byADLY.value[l][t] =
            AFE_TO_hour[l] * this.ANSDataTemplate.TL_DI_Dly.value[l][t]; // 국제선 총 공항 출발 연료 절감량

          this.ANSDataTemplate.CER_DDcost_byADLY.value[l][t] =
            this.ANSDatabase.carbonCredit.value[t] *
            this.ANSDataTemplate.CER_DDamount_byADLY.value[l][t]; // 국내선 총 공항 출발 탄소배출 절감비용
          this.ANSDataTemplate.CER_DIcost_byADLY.value[l][t] =
            this.ANSDatabase.carbonCredit.value[t] *
            this.ANSDataTemplate.CER_DIamount_byADLY.value[l][t]; // 국제선 총 공항 출발 탄소배출 절감비용
          this.ANSDataTemplate.FR_DDcost_byADLY.value[l][t] =
            this.ANSDatabase.fuelCredit.value[t] *
            this.ANSDataTemplate.FR_DDamount_byADLY.value[l][t]; // 국내선 총 공항 출발 연료 절감비용
          this.ANSDataTemplate.FR_DIcost_byADLY.value[l][t] =
            this.ANSDatabase.fuelCredit.value[t] *
            this.ANSDataTemplate.FR_DIamount_byADLY.value[l][t]; // 국제선 총 공항 출발 연료 절감비용
        }
      }
      ...
      for (let l = 0; l < MAX; l++) {
        ...
        for (let t = 0; t < YEAR; t++) {
          ...
          Avg_AD_Dly[l][t] =
            this.ANSDatabase.Time_AD_Dly.value[l] *
            Dly_Sav[t] *
            this.ANSDatabase.DLY_AD_ANSshare.value[l]; // 국내선 도착 지연감소 시간
          ...
          let FIR_LD_Share =
            this.ANSDatabase.Time_ILD.value[l] /
            (this.ANSDatabase.Time_AIRoute.value[l] +
              this.ANSDatabase.Time_ILD.value[l]); // 국제선 도착 국내 공역 운항 시간 중 착륙과정 비율

          Avg_AI_Dly[l][t] =
            this.ANSDatabase.Time_AI_Dly.value[l] *
            Dly_Sav[t] *
            this.ANSDatabase.DLY_AI_ANSshare.value[l] *
          ...
          this.ANSDataTemplate.TL_AD_Dly.value[l][t] =
            this.ANSDatabase.N_AD_Flght.value[l][t] * Avg_AI_Dly[l][t]; // 국내선 총 도착 지연시간 절감
          this.ANSDataTemplate.TL_AI_Dly_LD.value[l][t] =
            this.ANSDatabase.N_AI_Flght.value[l][t] * Avg_AI_Dly_LD[l][t]; // 국제선 총 도착 착륙과정 지연시간 절감
          this.ANSDataTemplate.TL_AI_Dly_R.value[l][t] =
            this.ANSDatabase.N_AI_Flght.value[l][t] * Avg_AI_Dly_R[l][t]; // 국제선 총 도착 항로과정 지연시간 절감
          this.ANSDataTemplate.TL_AI_Dly.value[l][t] =
            this.ANSDataTemplate.TL_AI_Dly_LD.value[l][t] +
            this.ANSDataTemplate.TL_AI_Dly_R.value[l][t]; // 국제선 총 도착 지연시간 절감
        }
      }
      ...
      for (let l = 0; l < MAX; l++) {
        for (let t = 0; t < YEAR; t++) {
          this.ANSDataTemplate.CER_ADamount_byADLY.value[l][t] =
            ACE_LD_hour[l] * this.ANSDataTemplate.TL_AD_Dly.value[l][t]; // 국내선 총 도착 탄소배출 절감량
          this.ANSDataTemplate.CER_AI_LDamount_byADLY.value[l][t] =
            ACE_LD_hour[l] * this.ANSDataTemplate.TL_AI_Dly_LD.value[l][t]; // 국제선 총 공항 도착(착륙) 탄소배출 절감량
          this.ANSDataTemplate.CER_AI_Ramount_byADLY.value[l][t] =
            FCE_hour[l] * this.ANSDataTemplate.TL_AI_Dly_R.value[l][t]; // 국제선 총 도착 항로 탄소배출 절감량
          ...
          this.ANSDataTemplate.FR_ADamount_byADLY.value[l][t] =
            AFE_TO_hour[l] * this.ANSDataTemplate.TL_AD_Dly.value[l][t]; // 국내선 총 도착 연료 절감량
          this.ANSDataTemplate.FR_AI_LDamount_byADLY.value[l][t] =
            AFE_TO_hour[l] * this.ANSDataTemplate.TL_AI_Dly_LD.value[l][t]; // 국제선 총 공항 도착(착륙) 연료 절감량
          this.ANSDataTemplate.FR_AI_Ramount_byADLY.value[l][t] =
            FFE_hour[t] * this.ANSDataTemplate.TL_AI_Dly_R.value[l][t]; // 국제선 총 도착 항로 연료 절감량
          ...
          this.ANSDataTemplate.CER_ADcost_byADLY.value[l][t] =
            this.ANSDatabase.carbonCredit.value[t] *
            this.ANSDataTemplate.CER_ADamount_byADLY.value[l][t]; // 국내선 총 공항 도착 탄소배출 절감비용
          this.ANSDataTemplate.CER_AI_LDcost_byADLY.value[l][t] =
            this.ANSDatabase.carbonCredit.value[t] *
            this.ANSDataTemplate.CER_AI_LDamount_byADLY.value[l][t]; // 국제선 총 공항 도착(착륙) 탄소배출 절감비용
          this.ANSDataTemplate.CER_AI_Rcost_byADLY.value[l][t] =
            this.ANSDatabase.carbonCredit.value[t] *
            this.ANSDataTemplate.CER_AI_Ramount_byADLY.value[l][t]; // 국제선 총 도착 항로 탄소배출 절감비용
          ...
          this.ANSDataTemplate.FR_ADcost_byADLY.value[l][t] =
            this.ANSDatabase.fuelCredit.value[t] *
            this.ANSDataTemplate.FR_ADamount_byADLY.value[l][t]; // 국내선 총 공항 도착 연료 절감비용
          this.ANSDataTemplate.FR_AI_LDcost_byADLY.value[l][t] =
            this.ANSDatabase.fuelCredit.value[t] *
            this.ANSDataTemplate.FR_AI_LDamount_byADLY.value[l][t]; // 국제선 총 공항 도착 연료 절감비용
          this.ANSDataTemplate.FR_AI_Rcost_byADLY.value[l][t] =
            this.ANSDatabase.fuelCredit.value[t] *
            this.ANSDataTemplate.FR_AI_Ramount_byADLY.value[l][t]; // 국제선 총 공항 도착 연료 절감비용
      ...
      for (let l = 0; l < MAX; l++) {
        ...
        for (let t = 0; t < YEAR; t++) {
          this.ANSDataTemplate.OPR_ADcost_DLY.value[l][t] =
            this.ANSDataTemplate.TL_AD_Dly.value[l][t] *
            this.ANSDatabase.OP_DDcost.value[t]; // 국내선 지연감소 운항비 절감
          this.ANSDataTemplate.OPR_DIcost_DLY.value[l][t] =
            this.ANSDataTemplate.TL_DI_Dly.value[l][t] *
            this.ANSDatabase.OP_DIcost.value[t]; // 국제선 출발지연 감소 운항비 절감
          this.ANSDataTemplate.OPR_AIcost_DLY.value[l][t] =
            this.ANSDataTemplate.TL_AI_Dly.value[l][t] *
            this.ANSDatabase.OP_ADcost.value[t]; // 국제선 도착지연 감소 운항비 절감
        }
      }
      ...
      for (let l = 0; l < MAX; l++) {
        for (let t = 0; t < YEAR; t++) {
          this.ANSDataTemplate.BNF_AD_PSG.value[l][t] =
            ...
            this.ANSDataTemplate.TL_AD_Dly.value[l][t] *
            this.ANSDatabase.DLY_AD_cost_PSG.value[l][t]; // 국내선 도착지연 감소편익(여객)
          this.ANSDataTemplate.BNF_AI_PSG.value[l][t] =
            this.ANSDatabase.PSG_AI.value[l][t] *
            this.ANSDataTemplate.TL_AI_Dly.value[l][t] *
            this.ANSDatabase.DLY_AI_cost_PSG.value[l][t]; // 국제선 도착지연 감소편익(여객)
      ...
      for (let l = 0; l < MAX; l++) {
        for (let t = 0; t < YEAR; t++) {
          Time_Pass[t] = Flt_Sav_2[t] * this.ANSDatabase.Time_Pass_S.value; // 영공통과 시간 절감
          FTR_Pass[t] = this.ANSDatabase.N_Pass_Flght.value[t] * Time_Pass[t]; // 총 영공통과 운항시간 절감
          ...
          this.ANSDataTemplate.CER_cost_byAFT.value[t] =
            this.ANSDatabase.carbonCredit.value[t] *
            this.ANSDataTemplate.CER_amount_byAFT.value[t]; // 총 영공통과 탄소배출 비용 감소
          ...
          this.ANSDataTemplate.FR_cost_byAFT.value[t] =
            this.ANSDatabase.fuelCredit.value[t] *
            this.ANSDataTemplate.FR_amount_byAFT.value[t]; // 총 영공통과 연료비 절감}
        }
      }
    },
  },
